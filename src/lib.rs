...
impl std::fmt::Display for PixelSnapperError {
...
impl std::error::Error for PixelSnapperError {}
...
impl ProcessedImage {
    ...
    pub fn pixel_size_override(&self) -> bool {
        self.pixel_size_override
    }
...
pub fn process_image_common(input_bytes: &[u8], config: Option<Config>) -> Result<ProcessedImage> {
...
pub fn process_image_to_bytes(input_bytes: &[u8], config: Option<Config>) -> Result<Vec<u8>> {
    process_image_common(input_bytes, config).map(|processed| processed.output_bytes)
}

#[cfg(not(target_arch = "wasm32"))]
pub fn process_file(input_path: &Path, output_path: &Path, config: &Config) -> Result<ProcessedImage> {
